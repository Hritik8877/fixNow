        ...
        <div className="hidden md:flex items-center gap-2 bg-zinc-100/50 dark:bg-zinc-800/50 p-1 rounded-2xl border border-outline-variant">
          {links.map(link => (
            <Link key={link.to} to={link.to} data-testid={`nav-link-${link.label.toLowerCase()}`}
              className={`px-6 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${isActive(link.to) ? 'bg-white dark:bg-zinc-900 text-foreground shadow-sm' : 'text-zinc-500 hover:text-foreground hover:bg-white/50 dark:hover:bg-zinc-800/50'}`}>
              {link.label}
            </Link>
          ))}
        </div>
